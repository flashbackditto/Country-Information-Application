...
let popButton = document.getElementById("clickMe").addEventListener('click', () => {
  removeLast();
  popGetThenSort();
});
...
//Clear previously created elements to make room for new elements
 let myNode = document.getElementById("root");
  for (let i = myNode.childNodes.length - 1; i >= 0; i--) {
     myNode.removeChild(myNode.childNodes[i]);
  }
}

function popGetThenSort(){
    ...
    byPop.forEach(country => {
     ...
     //Makes card a link to relevant wiki page
      ...
      if (country.area === null) {
        let countryNode = document.getElementById("root");
...
let sizeButton = document.getElementById("size").addEventListener('click', () => {
  removeLast();
  areaGetThenSort();
});
...
function areaGetThenSort (e) {
  ...
  if (request.status >= 200 && request.status < 400) {  //optional if statement to console log if theres a 404 not found error or otherwise
    ...
    byArea.forEach(country => {
      ...
      //creates a paragraph and the text to population
      const p = document.createElement('p');
      country.population = country.population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
      p.textContent = `Population: ${country.population}`;
      ...
      //makes sorted element stand out
  ...
  let alphabeticalButton = document.getElementById("alphabetical").addEventListener('click', () => {
    removeLast();
    defaultSort();
  });
